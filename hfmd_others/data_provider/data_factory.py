...
# from pred_dataset import hand_foot

def get_data(data_path='/home/Paradise/UPTST/dataset',
             data_name='cq_hfmd_2015_21.csv',seq_len=384,pre_len=384,batch_size=64,freq='d',
             split_ratio=[0.7,0.2,0.1],num_workers=0,scale=True,tasks='MS_1',
             ):

    dataset=hand_foot
    train_dataset=dataset(root_path=data_path,data_type='train', size=[seq_len,pre_len],split_ratio=split_ratio,
                  data_path=data_name,tasks=tasks,freq=freq,
                 scale=scale)

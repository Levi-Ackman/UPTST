...
def get_min_mae_mse_weighted(seed, task,model, cro='S'):
    seq_len, pre_len = task

    path_str = f"./test_dict/{seed}/{seq_len}->{pre_len}/{cro}/{model}"
...
if __name__ == "__main__":
    csv_filename = "other_results.csv"
            ...
            for j in range(6):
                for pre in pre_opt:
                    ...
                    for seed in random_seeds:
                        ...
                        for seq in seq_opt:
                                task = [seq, pre]
                                mae, _, mse, _ = get_min_mae_mse_weighted(seed=seed, task=task, model=model_dict[j+1],
                                                                           cro=cro_dict[i + 1])
                                if best_mae > mae:
                                    best_mae = mae
